...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.programs.update", [$program->slug]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" 
                                        style="width: 100%; height: 300px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" id="editor">{{ $program->description }}</textarea>
            ...
            <div class="form-group dynamic_field">
                ...
                @if(count($program->grades))
                    @foreach ($program->grades as $grade)
                        <input class="form-control {{ $errors->has('grades') ? 'is-invalid' : '' }}" type="text" name="grades[]" id="grades" value="{{ old('grades', $grade->title) }}" required>                    
                    @endforeach
                @else
