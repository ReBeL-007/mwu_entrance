                ...
                <tbody>
                    @foreach($programs as $key => $program)
                        <tr data-entry-id="{{ $program->id }}">
                            ...
                            <td>
                                @can('program-show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.programs.show', $program->slug) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('program-edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.programs.edit', $program->slug) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('program-delete')
                                    <form action="{{ route('admin.programs.destroy', $program->slug) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  $('.datatable-program:not(.ajaxTable)').DataTable({ buttons: dtButtons })
