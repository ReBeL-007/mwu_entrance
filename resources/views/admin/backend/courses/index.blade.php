                ...
                <tbody>
                    @foreach($courses as $key => $course)
                        <tr data-entry-id="{{ $course->id }}">
                            ...
                            <td>
                                @can('course-show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.courses.show', $course->slug) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('course-edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.courses.edit', $course->slug) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('sub-access')
                                <a class="btn btn-xs btn-primary" href="{{ route('admin.subs.courseSubjects', $course->id) }}">
                                    View {{ trans('cruds.sub.title') }}
                                </a>
                                @endcan

                                @can('course-delete')
                                    <form action="{{ route('admin.courses.destroy', $course->slug) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
