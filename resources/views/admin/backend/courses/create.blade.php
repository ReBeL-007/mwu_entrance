    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.course.fields.abbreviation_helper') }}</span>
                
            </div>

            {{-- <div class="form-group">
                <label class="" for="duration">{{ trans('cruds.course.fields.duration') }} (in Year)</label>
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="number" name="duration" id="duration" value="{{ old('duration', '') }}">
                    ...
                    <div class="col-md-11 row">
                        <select class="form-control select2 {{ $errors->has('programtypes') ? 'is-invalid' : '' }}" name="programtypes[]" id="programtypes" multiple required>
                            @foreach($programtypes as $id => $programtypes)
                                <option value="{{ $id }}" {{ in_array($id, old('programtypes', [])) ? 'selected' : '' }}>{{ $programtypes }}</option>
                            @endforeach
                        </select>
                    </div>
