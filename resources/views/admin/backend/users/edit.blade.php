    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.users.update', [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mobile') ? 'is-invalid' : '' }}" type="number" name="mobile" id="mobile" value="{{ old('mobile', $user->mobile) }}" >
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('groups') ? 'is-invalid' : '' }}" name="groups[]" id="groups" multiple >
                    @foreach($groups as $id => $groups)
                        <option value="{{ $id }}" {{ (in_array($id, old('groups', [])) || $user->groups->contains($id)) ? 'selected' : '' }}>{{ $groups }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('form_charge') ? 'is-invalid' : '' }}" type="number" name="form_charge" id="form_charge" value="{{ old('form_charge', $user->form_charge) }}" >
