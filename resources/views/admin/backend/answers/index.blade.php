...
@can('answer-create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-primary" href="{{ route("admin.answers.create") }}">
                    ...
                    @foreach($answers as $key => $answer)
                        <tr data-entry-id="{{ $answer->id }}">
                            ...
                            <td>
                                @can('answer-download')
                                    @if($answer->files)
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.answers.download', $answer->files) }}">
                                ...
                                @if(! $answer->is_uploaded && ! $answer->expired)
                                <a class="btn btn-xs btn-info" href="{{ route('admin.answers.tus', $answer->hash) }}">
                                    Add Answer
                                </a>
                                @endif
                                @can('answer-delete')
...
@endcan
  ...
  $('.datatable-answer:not(.ajaxTable)').DataTable({ buttons: dtButtons })
